...
// --- 1. RESILIENT PARSING ---
// We use a generic "Envelope" to capture EVERYTHING GlazeWM sends.
#[derive(Deserialize, Debug, Clone)]
struct GlazeEnvelope {
  #[serde(rename = "messageType")]
  message_type: String,
  // Use Value so we don't crash if the 'data' shape changes
...
struct Workspace {
  name: String,
  // FIXED: Accept either "focused" OR "hasFocus"
  #[serde(default, alias = "hasFocus")]
  focused: bool,

  // FIXED: Accept either "visible" OR "isDisplayed"
...
fn main() -> eframe::Result<()> {
  ...
  eframe::run_native(
    ...
    native_options,
    Box::new(|cc| {
      // 1. Get a handle to the UI Context so threads can wake it up
      let ctx = cc.egui_ctx.clone();
      let ctx_bat = cc.egui_ctx.clone(); // Clone again for battery thread

      let (ws_tx, ws_rx) = channel();
      let (bat_tx, bat_rx) = channel();
      // Inside main, before spawning threads:
      let repaint_signal = Arc::new(AtomicBool::new(false));
      let repaint_signal_ws = repaint_signal.clone();

      // --- WEBSOCKET THREAD (Now with Context) ---
      std::thread::spawn(move || {
        let url = Url::parse("ws://localhost:6123").unwrap();
        loop {
          // println!("Connecting to GlazeWM...");
      ...
      std::thread::spawn(move || {
        loop {
          if repaint_signal.swap(false, Ordering::Relaxed) {
            ctx.request_repaint(); // Wake up UI only if data changed!
          }
          // Cap updates to 10 FPS (100ms). This beats Zebar's efficiency massively.
          std::thread::sleep(std::time::Duration::from_millis(100));
        }
      });
    
      // --- BATTERY THREAD (Now with Context) ---
      std::thread::spawn(move || {
...
struct MyTaskbar {
  ...
  //rx: std::sync::mpsc::Receiver<Vec<Workspace>>,
}
      ...
      .show(ctx, |ui| {
        ui.horizontal(|ui| {
          ...
          for ws in &self.workspaces {
            ...
            if ui.interact(resp.rect, resp.id, egui::Sense::click()).clicked() {
              let _ = std::process::Command::new("glazewm")
                .args(["command", "focus", "--workspace", &ws.name])
                .spawn();
            }
          }
          ui.separator();
